defmodule ElixirOntologies.SHACL.W3CTestRunner do
  ...
  def parse_test_file(file_path) do
    ...
    with {:ok, graph} <- RDF.Turtle.read_file(file_path, base_iri: base_iri),
         {:ok, test_id} <- extract_test_id(graph),
         {:ok, label} <- extract_label(graph, test_id),
         {:ok, type} <- extract_type(graph, test_id),
         {:ok, expected_conforms} <- extract_expected_conforms(graph, test_id),
         expected_result_count <- extract_expected_result_count(graph, test_id) do
      {:ok,
       %{
         id: test_id,
         label: label,
